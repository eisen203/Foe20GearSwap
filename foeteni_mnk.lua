...
windower.raw_register_event('incoming chunk', function(id,original,modified,injected,blocked)
    ...
    if id == 0x028 then
        ...
        if packet.Actor == self.id and category == 1 and state.AutoWS.value == 'true' then
			...
			if state.AutoWSList.value == 'Howling' and player.tp >= 1150 and wsnum == 0 then
                    send_command('input /ws Howling Fist <t>')
                    wsnum = 0
			end

			if state.AutoWSList.value == 'Omen' and player.tp >= 1000 and wsnum == 0 then
                    send_command('input /ws One Inch Punch <t>')
                    wsnum = 0
			end	

			if state.AutoWSList.value == 'Howling' and player.tp >= 1000 and wsnum == 0 then
                    send_command('input /ws Howling Fist <t>')
                    wsnum = 0
			end	

			if state.AutoWSList.value == 'Victory' and player.tp >= 1000 and wsnum == 0 then
			...
			if state.AutoWSList.value == '4step1' and player.tp >= 1000 and wsnum == 0 then
                ...
                elseif state.AutoWSList.value == '4step1' and player.tp >= 1000 and wsnum == 1 then 
                    send_command('input /ws Howling Fists <t>')
                    wsnum = wsnum + 1
			...
			if state.AutoWSList.value == '4step2' and player.tp >= 1000 and wsnum == 0 then
                    send_command('input /ws Tornado Kick <t>')
                    wsnum = wsnum + 1
 
                elseif state.AutoWSList.value == '4step2' and player.tp >= 1000 and wsnum == 1 then 
                    send_command('input /ws Shijin Spiral <t>')
                    wsnum = wsnum + 1
 
                elseif state.AutoWSList.value == '4step2' and player.tp >= 1000 and wsnum == 2 then
                    send_command('input /ws Shijin Spiral <t>')
                    wsnum = wsnum + 1
 
                elseif state.AutoWSList.value == '4step2' and player.tp >= 1000 and wsnum == 3 then
                    send_command('input /ws Shijin Spiral  <t>')
                    wsnum = 0
					add_to_chat(100," WS #0 nigga")
			end
