    ...
    if id == 0x028 then
        ...
        if packet.Actor == self.id and category == 1 and state.AutoWS.value == 'true' then
			...
			if state.AutoWSList.value == 'Omen' and player.tp >= 1000 and wsnum == 0 then
                    send_command('input /ws Leg Sweep <t>')
                    wsnum = 0
			end	

			if state.AutoWSList.value == 'Impulse spam' and player.tp >= 2000 and wsnum == 0 then
                    send_command('input /ws Impulse Drive  <t>')
                    wsnum = 0
			end	

			if state.AutoWSList.value == 'Camlann spam' and player.tp >= 1000 and wsnum == 0 then
                    send_command('input /ws Camlann\'s Torment <t>')
                    wsnum = 0
			end		

			if state.AutoWSList.value == 'Savage spam' and player.tp >= 1400 and wsnum == 0 then
                    send_command('input /ws Savage Blade  <t>')
                    wsnum = 0
			end				

			
			if state.AutoWSList.value == '4stepLight' and player.tp >= 1000 and wsnum == 0 then
                    send_command('input /ws Stardiver <t>')
                    wsnum = wsnum + 1
 
                elseif state.AutoWSList.value == '4stepLight' and player.tp >= 1000 and wsnum == 1 then 
                    send_command('input /ws Camlann\'s Torment <t>')
                    wsnum = wsnum + 1
 
                elseif state.AutoWSList.value == '4stepLight' and player.tp >= 1000 and wsnum == 2 then
                    send_command('input /ws Drakesbane <t>')
                    wsnum = wsnum + 1
 
                elseif state.AutoWSList.value == '4stepLight' and player.tp >= 1000 and wsnum == 3 then
                    send_command('input /ws Camlann\'s Torment  <t>')
                    wsnum = 0
			...
			if state.AutoWSList.value == '4stepDark' and player.tp >= 1000 and wsnum == 0 then
                    send_command('input /ws Penta Thrust <t>')
                    wsnum = wsnum + 1
 
                elseif state.AutoWSList.value == '4stepDark' and player.tp >= 1000 and wsnum == 1 then 
                    send_command('input /ws Drakesbane <t>')
                    wsnum = wsnum + 1
 
                elseif state.AutoWSList.value == '4stepDark' and player.tp >= 1000 and wsnum == 2 then
                    send_command('input /ws Sonic Thrust <t>')
                    wsnum = wsnum + 1
 
                elseif state.AutoWSList.value == '4stepDark' and player.tp >= 1000 and wsnum == 3 then
                    send_command('input /ws Impulse Drive  <t>')
                    wsnum = 0
					add_to_chat(100," WS #0 nigga")
			end
