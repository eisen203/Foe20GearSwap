...
--[[windower.register_event('incoming text', function(original, modified, original_mode, modified_mode, blocked)
    if string.find(modified,"Skillchain: Light") then
        windower.add_to_chat(10, "Light SC." )
        if state.AutoBurst.value == 'ON' then
            send_command('input /ma Aero V <bt>')
			--send_command('wait 6; input /ma Aero V <bt>')
        end
    end
    if string.find(modified,"Light:") then
        windower.add_to_chat(10, "Light SC 2." )
        if state.AutoBurst.value == 'ON' then
            send_command('input /ma Aero VI <bt>')
			--send_command('wait 6; input /ma Fire V <bt>')
        end
    end
    if string.find(modified,"Radiance:") then
        windower.add_to_chat(10, "Radiance SC 2." )
        if state.AutoBurst.value == 'ON' then
            send_command('input /ma Fire VI <bt>')
        end
    end
    if string.find(modified,"Skillchain: Radiance") then
        windower.add_to_chat(10, "Radiance SC." )
        if state.AutoBurst.value == 'ON' then
            send_command('input /ma Aero VI <bt>')
        end
    end
    if string.find(modified,"Fragmentation:") then
        --windower.add_to_chat(10, "Fragmentation SC 2." )
        if state.AutoBurst.value =='ON' then
            send_command('input /ma Thunder V <bt>')
        end
    end

if string.find(modified,"Fusion:") then
        --windower.add_to_chat(10, "Fusion SC." )
        if state.AutoBurst.value == 'ON' then
            send_command('input /ma Fire V <bt>')
        end
    end
    if string.find(modified,"Darkness:") then
        --windower.add_to_chat(10, "Darkness SC." )
        if state.AutoBurst.value == 'ON' then
            send_command('input /ma Blizzard V <bt>')
        end
    end
    if string.find(modified,"Distortion:") then
        --windower.add_to_chat(10, "Distortion SC." )
        if state.AutoBurst.value == 'ON' then
            send_command('input /ma Blizzard V <bt>')
        end
    end
    if string.find(modified,"uses Savage Blade") then
        --windower.add_to_chat(10, "use Full Break" )     
        if state.AutoBurst.value == 'ON' then
            send_command('input /Myrkr')
        end
    end
    if string.find(modified,"readies Savage Blade") then
        windower.add_to_chat(10, "ready Full Break" )
        if state.AutoBurst.value == 'ON' then
            --send_command('input /ma Blizzard V <bt>')
        end
    end
