...
windower.raw_register_event('incoming chunk', function(id,original,modified,injected,blocked)
    ...
    if id == 0x028 then
        ...
        if packet.Actor == self.id and category == 1 and state.AutoWS.value == 'true' then
			...
			if state.AutoWSList.value == 'CDC' and player.tp >= 1000 and wsnum == 0 then
                    send_command('input /ws Chant du Cygne <t>')
                    wsnum = 0
			end

			if state.AutoWSList.value == 'Savage' and player.tp >= 1000 and wsnum == 0 then
                    send_command('input /ws Savage Blade <t>')
                    wsnum = 0
			end
			
			if state.AutoWSList.value == 'Expiacion' and player.tp >= 1000 and wsnum == 0 then
                    send_command('input /ws Expiacion <t>')
                    wsnum = 0
			end

			if state.AutoWSList.value == 'Halo' and player.tp >= 1000 and wsnum == 0 then
                    send_command('input /ws Black Halo <t>')
                    wsnum = 0
			end
			
			if state.AutoWSList.value == 'Vagary6step' and player.tp >= 1000 and wsnum == 0 then
                    send_command('input /ws Vorpal Blade <t>')
                    wsnum = wsnum + 1
 
                elseif state.AutoWSList.value == 'Vagary6step' and player.tp >= 1000 and wsnum == 1 then 
                    send_command('input /ws Red Lotus Blade <t>')
                    wsnum = wsnum + 1
 
                elseif state.AutoWSList.value == 'Vagary6step' and player.tp >= 1000 and wsnum == 2 then
                    send_command('input /ws Vorpal Blade <t>')
                    wsnum = wsnum + 1
 
                elseif state.AutoWSList.value == 'Vagary6step' and player.tp >= 1000 and wsnum == 3 then
                    send_command('input /ws Red Lotus Blade  <t>')
                    wsnum = wsnum + 1
               
			    elseif state.AutoWSList.value == 'Vagary6step' and player.tp >= 1000 and wsnum == 4 then
                    send_command('input /ws Vorpal Blade  <t>')
                    wsnum = wsnum + 1
					
			    elseif state.AutoWSList.value == 'Vagary6step' and player.tp >= 1000 and wsnum == 5 then
                    send_command('input /ws Circle Blade  <t>')
                    wsnum = 0
